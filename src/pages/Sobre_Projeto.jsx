...
// Imports para renderização do Totem 3D
import { Canvas } from '@react-three/fiber';
import { OrbitControls, Stage, Center } from '@react-three/drei';

import { Model as Totem3D } from "../components/Totem_3D_Oficial.jsx"; // ADICIONADA A EXTENSÃO EXPLICITA .jsx

// Imports de assets mantidos
...
function Sobre_Projeto() {
  // Ajuste esse número para definir em qual etapa o projeto está atualmente.
  // 0 = Pesquisa, 1 = Idealização, 2 = UI/UX, 3 = Protótipo, 4 = IA's, 5 = Hardware, 6 = Testes, 7 = Expansão
  const [currentStepIndex, setCurrentStepIndex] = useState(4); 

  // Estrutura de dados para a linha do tempo (unindo títulos e imagens correspondentes)
  const timelineSteps = [
  ...
  {/* Ajustamos a câmera em Y para 1.0 (antes 1.5) e Z para 8.0 para ter uma visão mais frontal e controlada */}
  <Canvas camera={{ position: [0, 1.0, 8.0], fov: 45 }}>
    <ambientLight intensity={0.8} />
    <directionalLight position={[10, 10, 5]} intensity={1.5} />
    
    <Suspense fallback={null}>
      {/* O Stage ainda cuida da iluminação e do ambiente de cidade */}
      <Stage environment="city" intensity={0.5} adjustCamera={false}>
        <Center>
          {/* O SEGREDO ESTÁ AQUI: Criamos um <group> para centralizar o ponto */}
          {/* E deslocamos o totem ligeiramente para CIMA em Y (position={[0, 0.5, 0]}) */}
          {/* Isso afasta a base do totem do chão invisível do Stage */}
